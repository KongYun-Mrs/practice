...
def AddGroup(oneDev):
    ...
    url = host + '/func/web_main/api/pf_policy/pf_policy/pf_policy/securitypolicylist'

    code, content = http_request(url=url, method='POST', data=data, authorization=authorization)
    if code == 201:
        content = u"创建成功"
    return content
